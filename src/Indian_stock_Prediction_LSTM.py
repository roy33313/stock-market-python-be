...
for dirname, _, filenames in os.walk('D:/Desktop/To_send/To_send/Data1'):
    for filename in filenames:
        ...
        dataframes.append(pd.read_csv(
            os.path.join(dirname, filename), index_col=0))
...
j = 0
# for i in range(0,1):
dataframes[['Adj Close', 'MA for 10 days', 'MA for 50 days', 'MA for 100 days',
            'MA for 365 days']].plot(ax=axes[k, j]).set_title(f"{stock_name}")
j = j+1
if (j == 5):
    k = 1
    j = 0
...
j = 0
# for i in range(0,1):
dataframes[['Daily Return']].plot(
    ax=axes[k, j], linestyle='--', marker='o').set_title(f"{stock_name}")
j = j+1
if (j == 5):
    k = 1
    j = 0
...
# Test the function
TEST_DATA_LENGTH = 60
x_test, y_test, predictions, rmse = create_testing_data_set(
    lstm_model, scaler, training_data_len, TEST_DATA_LENGTH)
